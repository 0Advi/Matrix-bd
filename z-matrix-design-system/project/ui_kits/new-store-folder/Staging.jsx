...
// Critical timing KPI block — "draft → LOI" is the load-bearing metric
// of this module. Surfaced above the filter bar so it's visible at a glance.
...
const KpiTile = ({ label, value, sub, tone = 'neutral' }) => {
  const tones = {
    neutral: { color: 'var(--zm-fg)',  rule: 'var(--zm-fg-3)' },
    good:    { color: '#047857',       rule: '#047857' },
    warn:    { color: '#B45309',       rule: '#D97706' },
    bad:     { color: '#B91C1C',       rule: '#B91C1C' },
  }[tone] || { color: 'var(--zm-fg)', rule: 'var(--zm-fg-3)' };
  return (
    <div style={{
      flex: 1, minWidth: 140,
      padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 4,
      background: 'var(--zm-surface)', border: '1px solid var(--zm-line)', borderRadius: 10,
      borderTop: '2px solid ' + tones.rule,
    }}>
      <span style={{
      ...
      <span style={{
        ...
        fontSize: 22, fontWeight: 600, color: tones.color, lineHeight: 1.1,
      }}>{value}</span>
      {sub && (
        <span style={{ fontFamily: 'var(--zm-font-body)', fontSize: 11, color: 'var(--zm-fg-3)' }}>{sub}</span>
      )}
    </div>
  );
};

const StagingKpiStrip = ({ sites, role }) => {
  // Draft → LOI in days, computed across the visible scope of `sites`.
  const uploaded = sites.filter(s => s.loiUploaded);
  const draftToLoiDays = uploaded
    .map(s => daysBetweenISO(s.draftDate, s.loiUploadedAt))
  ...
  const dueSoon   = open.filter(s => {
    const remaining = s.expectedLoiDays - s.daysSinceApproval;
  ...
  return (
    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
      <KpiTile
        label="Median · draft → LOI"
        value={medDraftToLoi == null ? '—' : `${medDraftToLoi}d`}
        sub={uploaded.length ? `${uploaded.length} LOI${uploaded.length === 1 ? '' : 's'} uploaded` : 'no LOIs yet'}
      />
      <KpiTile
        label="LOI on-time rate"
        value={hitRate == null ? '—' : `${hitRate}%`}
        sub={uploaded.length ? `${onTime} on time · ${late} late` : 'pending uploads'}
        tone={hitRate == null ? 'neutral' : hitRate >= 80 ? 'good' : hitRate >= 50 ? 'warn' : 'bad'}
      />
      {role !== 'supervisor' && (
        <>
          <KpiTile
            label="Due in ≤ 3 days"
            value={String(dueSoon).padStart(2, '0')}
            sub="upload before timer expires"
            tone={dueSoon > 0 ? 'warn' : 'neutral'}
          />
          <KpiTile
            label="Past expected LOI"
            value={String(overdue).padStart(2, '0')}
            sub={overdue > 0 ? 'flagged for supervisor' : 'no overdue sites'}
      ...
      {role === 'supervisor' && (
        <KpiTile
          label="LOIs awaiting push"
          value={String(uploaded.filter(s => !s.pushed).length).padStart(2, '0')}
          sub="ready for Payments module"
        />
      )}
    </div>
  );
  ...
  return (
    <div className="zm-row" style={{
      ...
      <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
        <button onClick={() => onOpen(site)} title="View" className="zm-icon-btn" style={{
        ...
        {uploaded ? (
          ...
          <button onClick={() => onUpload(site)} className="zm-btn-primary" style={{
  ...
  return (
    <div className="zm-row" style={{
      ...
      <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', minWidth: 0 }}>
        <button onClick={() => onOpen(site)} title="View site" className="zm-icon-btn" style={{
          width: 32, height: 32, padding: 0, border: '1px solid var(--zm-line)', borderRadius: 7,
          background: 'transparent', color: 'var(--zm-fg)',
          cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
          flex: '0 0 32px',
        }}><EyeIcon size={14}/></button>
        <button onClick={() => onViewLOI(site)} title="View LOI" className="zm-icon-btn" style={{
          width: 32, height: 32, padding: 0, border: '1px solid var(--zm-line)', borderRadius: 7,
          background: 'transparent', color: 'var(--zm-fg)',
          cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
          flex: '0 0 32px',
        ...
        {pushed ? (
          ...
          <button onClick={() => onPush(site)} className="zm-btn-primary" style={{
...
const StagingView = ({ sites, role, onUpload, onOpen, onPush, onViewLOI }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <StagingKpiStrip sites={sites} role={role}/>
          ...
          <div>
            <div style={{
              ...
              <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', alignItems: 'center' }}>
                <span aria-hidden="true" style={{ width: 32, flex: '0 0 32px' }}/>
                <span aria-hidden="true" style={{ width: 32, flex: '0 0 32px' }}/>
                <span style={{ flex: '1 1 auto', minWidth: 100, textAlign: 'center' }}>Action</span>
              </div>
            </div>
